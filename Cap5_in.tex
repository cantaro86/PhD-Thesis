...
The writer (or buyer) of the option creates a portfolio at time $t_0$ with the purpose to hedge the option.
...
The value function (\ref{max_probl1}) has a cost functional as in (\ref{singular_cost_functional}) with $f$ and $h$ equal to zero. 
...
from the general equation (\ref{variational_inequality}). We omit the subscript of $V^j$ when the discussion refers to all the three different problems $j=0,w,b$.
\begin{itemize}
 ...
 \item $H(p) = \sup_{a \in \hat K} \bigl( p \cdot \kappa(t,x) \cdot a  \bigr) \quad \mbox{ and define } 
...
total wealth due to the transaction costs and therefore makes no sense. 
...
$ \pi(t_0^-) = \bigl ( L(t_0^-) , M(t_0^-) \bigr ) = (0,0)$.
...
in a small ball for a small $\Delta t$. This problem can be circumvented by relying on the
...
in this framework cannot exist.\\
...
$|X_{\tau_{\rho}} - x|$ may be greater than $\rho$. Lévy processes and, more in general, all the controlled processes with dynamics as in Eq. (\ref{controlled_SDE}), 
...
Since the dynamics of $\{S_t\}_{t>0}$ is stochastically continuous and by definition the control process $\pi$ cannot be the cause of the bankruptcy,
we can exclude an immediate jump out of the solvency region, i.e. $t_0 \not = \tau_{\SI}$.

The theorem (\ref{stochastic_theorem}) is very useful for problems of the type presented in Section \ref{Optimal_control_framework}). 
However we cannot use it in our framework. For problems with state dynamics as in (\ref{singular_SDE}), the presence of the singular control 
cannot guarantee that the process is contained in the ball $\mathcal{B}(x, \rho )$ for small $\Delta t$.
...
The proof follows the ideas of the approach of \cite{OkSu01}.

First of all we have to interpret the HJB equation (\ref{HJB1}), (\ref{lat_bound}), (\ref{terminal_conditions}) as a parabolic problem of the type (\ref{parabolic_PIDE}). 
...
Therefore the jumps $\Delta \pi(t)>0$ of the control can be divided into smaller jumps i.e.
$\Delta \pi(t) = \sum_i c_i \Delta \pi(t)$, with $c_i \geq 0$ and $\sum_i c_i = 1$,
such that after the action of each $c_i \Delta \pi(t)$, the process is contained in an arbitrarily small closed ball.  

Let us consider an $\epsilon$-optimal control $\{\pi_t\}_{t_0 \leq t \leq T}$. At the selected point $(\bar t,\bar x)$ there are two alternative cases:
\begin{enumerate}
 \item[\textbf{A}] $\Delta \pi(\bar t,\bar x) = 0$.\\
  Since $\pi$ is cádlág, there always exists a $\rho>0$ and a ball $\mathcal{B}(\bar x, \rho)$ such that $\Delta \pi(t,x) = 0$ in the interval $\bar t \leq t \leq \tau_{\rho}$ 
  for each $x \in \mathcal{B}(\bar x, \rho)$.
 \item[\textbf{B}] $\Delta \pi(\bar t,\bar x) > 0$.\\
 It is always possible to select the value $\rho < \Delta \pi(\bar t,\bar x)$ such that $\tau_{\rho} = \bar t$.
\end{enumerate}
The action of the control can be split in the following way. Let us indicate 
\begin{align*}
& x_0 = \bar x = (\bar b, \bar y, \bar s) = (b_0, y_0, s_0), \\
& x_{i+1} = x_{i} + \biggl( - (1+\theta_b) \bar s\, c_i  \Delta L(\bar t) + (1-\theta_s) \bar s\,c_i \Delta M(\bar t), c_i \Delta L(\bar t) - c_i \Delta M_{\bar t}, 0 \biggr).  
\end{align*}
For each $x_i$, we choose a $\rho_i>0$. We can now define the coefficients $c_i$ as:
\begin{align*}
 c_i =& \inf \biggl\{ c \in [0,1] \, : \\
 & x_{i} + \biggl( - (1+\theta_b) \bar s\, c  \Delta L(\bar t) + (1-\theta_s) \bar s\,c \Delta M(\bar t), c \Delta L(\bar t) - c \Delta M_{\bar t}, 0 \biggr)
...
\begin{align}\label{first_inequality}
 0 \; \leq & \; \E_{\bar x} \biggl[ \phi(\tau_{\rho}, X^{\pi}_{\tau_{\rho}}) - \phi(\bar t, \bar x) \biggr] + \epsilon \\ \nonumber
    ...
    & + \sum_{\bar t \leq t_k \leq \tau_{\rho}} \biggl( \phi \bigl( t_k, B^{\pi}(t_k^-) - (1+\theta_b) S(t_k^-)\, \Delta L(t_k), Y^{\pi}(t_k^-) + \Delta L(t_k), S(t_k) \bigr) \\ \nonumber 
    & \hspace{4em} - \phi \bigl(t_k, B^{\pi}(t_k^-), Y^{\pi}(t_k^-), S(t_k) \bigr) \biggr) \\ \nonumber
    & + \sum_{\bar t \leq t_k \leq \tau_{\rho}} \biggl( \phi \bigl( t_k, B^{\pi}(t_k^-) + (1-\theta_s) S(t_k^-)\, \Delta M(t_k), Y^{\pi}(t_k^-) - \Delta M(t_k), S(t_k) \bigr) \\
    & \hspace{4em} - \phi \bigl(t_k, B^{\pi}(t_k^-), Y^{\pi}(t_k^-), S(t_k) \bigr) \biggr) \biggr] + \epsilon
\end{align}
where $t_k$ denote the jump times of $\{\pi_t\}_{t \in [\bar t, \tau_{\rho}]}$ and we indicate the continuous parts of $\pi$ as
\begin{equation}
 L^c_t = L_t - \sum_{\bar t \leq t_k \leq \tau_{\rho}} \Delta L(t_k) \quad \mbox{ and } \quad  M^c_t = M_t - \sum_{\bar t \leq t_k \leq \tau_{\rho}} \Delta M(t_k).
\end{equation}

\textbf{Case A}: $\Delta \pi(t) = 0$ for each $\bar t \leq t \leq \tau_{\rho}$ and the previous equation becomes:
\begin{align}
 0 \; \leq & \; E_{\bar x} \biggl[ \int_{\bar t}^{\tau_{\rho}} -\LL \phi(t,X^{\pi}_t) dt \\ \nonumber
   ...
   <& \underbrace{\biggl( -\kappa\, \E_{\bar x} \bigl[ \tau_{\rho} - \bar t \bigr] - \kappa\, \E_{\bar x} \bigl[ L^c(\tau_{\rho})-L^c(\bar t) \bigr]  
   - \kappa\, \E_{\bar x} \bigl[ M^c(\tau_{\rho})-M^c(\bar t) \bigr]  \biggr)}_{<0} + \epsilon. 
\end{align}
Since this must hold true for all $\epsilon$, when taking the limit $\epsilon \to 0$ we get a contradiction.\\

\textbf{Case B}:
For $\tau_{\rho} = \bar t$, we can split the trade and consider only the first action of the control $c_0 \Delta \pi(\bar t)$ such that the state moves from $x_0$ to 
$x_1 \in \mathcal{B}(x_0, \rho_0)$.
Let us consider for simplicity the case $\Delta \pi(\bar t) = (\Delta L(\bar t), 0)$. The case with $\Delta \pi(\bar t) = (0, \Delta M(\bar t))$ is analogous. \\  
At this point we can choose $\rho_0$ such that $c_0 \Delta L(\bar t) = 2 \epsilon / \kappa$. 
For a fixed point $(\bar t, \bar x)$, we can choose $\epsilon$ small enough such that $2 \epsilon / \kappa < \Delta L(\bar t)$. 
If this was not possible because $\Delta L(\bar t)$ is always smaller than $2 \epsilon / \kappa$, then we have that 
as $\epsilon \to 0$ also $\Delta L(\bar t) \to 0$ and consequently $x_1 \to x_0$ 
and $\phi(\bar t, x_1) \to \phi(\bar t, x_0)$, leading to the previous case A.\\      
Using the DPP between the two states $x_0$ and $x_1$, the equation (\ref{first_inequality}) becomes: 
\begin{align*}
 0 \; \leq & \; \phi(\bar t, x_1) - \phi(\bar t, x_0) + \epsilon \\
         = & \; \frac{\phi \bigl( \bar t, \bar b - (1+\theta_b) \bar s\, c_0 \Delta L(\bar t), \bar y + c_0 \Delta L(\bar t), \bar s \bigr) - \phi(\bar t, \bar x)}{c_0 \Delta L(\bar t)}  
            + \frac{\epsilon}{c_0 \Delta L(\bar t)} \\
...
\begin{align}\label{first_inequality2}
 0 \; \geq & \; \E_{\bar x} \biggl[ \phi(\tau, X^{\pi}_{\tau}) - \phi(\bar t, \bar x) \biggr] \\ \nonumber
    ...
    & + \sum_{\bar t \leq t_k \leq \tau} \biggl( \phi \bigl( t_k, B^{\pi}(t_k^-) - (1+\theta_b) S(t_k^-)\, \Delta L(t_k), Y^{\pi}(t_k^-) + \Delta L(t_k), S(t_k) \bigr) \\ \nonumber 
    & \hspace{4em} - \phi \bigl(t_k, B^{\pi}(t_k^-), Y^{\pi}(t_k^-), S(t_k) \bigr) \biggr) \\ \nonumber
    & + \sum_{\bar t \leq t_k \leq \tau} \biggl( \phi \bigl( t_k, B^{\pi}(t_k^-) + (1-\theta_s) S(t_k^-)\, \Delta M(t_k), Y^{\pi}(t_k^-) - \Delta M(t_k), S(t_k) \bigr) \\
...
\begin{enumerate}
 \item[\textbf{A}] There is a jump $\Delta \pi(\bar t)>0$ at the initial time $\bar t$, and then the process is continuous i.e. $\Delta \pi(t)=0$ for 
 $t \in (\bar t, \tau]$. We consider the two cases $\Delta L(\bar t) = l >0$ and $\Delta M(\bar t)=0$ or $\Delta L(\bar t) =0$ and $\Delta M(\bar t)=m>0$.
 \item[\textbf{B}] The control $\pi(t)$ is constant for all $t \in [\bar t, \tau]$.   
\end{enumerate}
\textbf{Case A:}
...
-\left(\frac{\partial \phi}{\partial y}(\bar t, \bar x)
 -(1+\theta_b) \bar s \frac{\partial \phi}{\partial b}(\bar t, \bar x)\right) \geq 0.
\end{equation*}
The third inequality can be obtained following the same steps but choosing $\Delta M(\bar t) = m >0$ and $\Delta L(\bar t)=0$.
