...
We can write the distance as $| T^{\rho}-\bar t | = f(\rho)$ where $f(\rho)$ is an increasing, non-negative function such that $\lim_{\rho \to 0} f(\rho) = 0$.
...
As done before, we can write $| T^{\delta}-\bar t | = f(\delta)$.
...
\end{equation}
A consequence of this lemma is that $\PP \bigl( X^{\pi^b}_{\tau_{\delta}} \not \in \mathcal{B}(\bar x, \rho) \bigr)$ can again be made arbitrarily small (see Corollary 
\ref{Corollary_P_exit}).\\
...
Let us express the terms in the sums in integral form. We obtain
...
by definition the control processes are non-decreasing functions.
...
\end{enumerate}
It follows that at least one expectation term in the penultimate line above is positive. When $\epsilon \to 0$, we obtain a contradiction.
\end{proof}
...
\begin{Corollary}\label{Corollary_P_exit}
